...
import user11681.spun.client.gui.widget.scalable.SpunWidgetSupplier;
...
import user11681.usersmanual.client.gui.widget.scalable.SpunWidgetSupplier;
...
public class SkillTab extends SoulboundTab {
    ...
    public SkillTab(final SoulboundComponent component, final List<ScreenTab> tabs) {
        super(Translations.MENU_SKILLS, component, tabs);
    ...
    public void init(final MinecraftClient client, final int width, final int height) {
        ...
        this.window = SpunWidgetSupplier.WINDOW.get();
    ...
    public void renderWindow(final MatrixStack stack, final int mouseX, final int mouseY, final float tickDelta) {
        ...
        TEXT_RENDERER.draw(stack, Translations.MENU_SKILLS, x + 8, y + 6, color);

        if (points > 0) {
            final Text text = new TranslatableText("%s: %s", Translations.MENU_UNSPENT_POINTS, points);
    ...
    protected void renderTooltip(final MatrixStack stack, final SkillContainer skill, final int centerX, final int centerY, final int offsetV) {
        ...
        if (size > 0) {
            ...
            if (!learned) {
                ...
                final Text plural = cost > 1 ? Translations.MENU_POINTS : Translations.MENU_POINT;
                string = new TranslatableText(Translations.MENU_SKILL_LEARN_COST.toString(), cost, plural);
            } else if (skill.canBeUpgraded()) {
                string = new TranslatableText("%s %s", Translations.MENU_LEVEL, skill.getLevel());
            }
