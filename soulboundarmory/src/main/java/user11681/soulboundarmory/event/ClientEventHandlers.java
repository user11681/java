...
public class ClientEventHandlers implements ClientListenerInitializer {
        ...
        if (player != null) {
            ...
            if (storage != null) {
                ...
                for (int index = 0, size = tooltip.size(); index < size; index++) {
                    ...
                    if (entry instanceof TranslatableText && ((TranslatableText) entry).getKey().equals("item.modifiers.mainhand")) {
                        startIndex += index;
                    }
                }
