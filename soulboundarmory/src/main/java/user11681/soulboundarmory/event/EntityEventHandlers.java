...
public class EntityEventHandlers implements CommonListenerInitializer {
    ...
    public static void onLivingAttack(final EntityDamageEvent.Pre event) {
        ...
        if (entity instanceof PlayerEntity && !entity.world.isClient && Components.WEAPON_COMPONENT.get(entity).getStorage(StorageType.GREATSWORD).getLeapForce() > 0) {
    ...
    public static void onLivingDeath(final LivingDeathEvent event) {
        ...
        if ((attacker instanceof PlayerEntity) && !attacker.world.isClient) {
            ...
            if (immediateSource instanceof SoulboundDaggerEntity) {
                ...
                displayName = ItemUtil.getEquippedItemStack(player, SoulboundItems.SOULBOUND_SWORD).getName();
            } else {
            ...
            if (storage != null) {
                ...
                final ConfigComponent configComponent = Components.CONFIG_COMPONENT.get(player);

                if (storage.incrementStatistic(EXPERIENCE, (int) Math.round(xp)) && configComponent.getLevelupNotifications()) {
                    ((PlayerEntity) attacker).sendMessage(new TranslatableText("message.soulboundarmory.levelup", displayName, storage.getDatum(LEVEL)), true);
                }
    ...
    public static void onLivingUpdate(final LivingTickEvent event) {
        final EntityData entityData = Components.ENTITY_DATA.get(event.getEntity());
    ...
    public static void onEnderTeleport(final EnderTeleportEvent event) {
        final EntityData component = Components.ENTITY_DATA.get(event.getEntity());
