...
public class SoulboundArmoryCommand {
    protected static final DynamicCommandExceptionType NO_ITEM_EXCEPTION = new DynamicCommandExceptionType((final Object player) -> new TranslatableText(Translations.COMMAND_NO_ITEM.getKey(), ((PlayerEntity) player).getName()));
        ...
        dispatcher.register((literal("sba").requires((final ServerCommandSource source) -> source.hasPermissionLevel(2)))
                ...
                .then(literal("reset")
                        .then(argument("storage", storages()).executes((final CommandContext<ServerCommandSource> context) -> reset(context, false, false))
                                .then(argument("category", registry(Registries.CATEGORY)).executes((final CommandContext<ServerCommandSource> context) -> reset(context, false, true))
    ...
    protected static ItemStorage<?> getStorage(final PlayerEntity player) throws CommandSyntaxException {
        for (final SoulboundComponent component : Components.getComponents(player)) {
