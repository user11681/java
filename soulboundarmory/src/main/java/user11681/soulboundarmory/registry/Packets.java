...
public class Packets {
    ...
    public static void register() {
        if (SoulboundArmory.ENVIRONMENT == EnvType.CLIENT) {
            for (final Map.Entry<Identifier, Packet> entry : Registries.PACKET.entries()) {
                SoulboundArmoryClient.PACKET_REGISTRY.register(entry.getKey(), entry.getValue());
            }
        }

        for (final Map.Entry<Identifier, Packet> entry : Registries.PACKET.entries()) {
            SoulboundArmory.PACKET_REGISTRY.register(entry.getKey(), entry.getValue());
        }
    }
}
